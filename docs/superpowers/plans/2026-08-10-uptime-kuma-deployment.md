...
### Fix Round 1 - Commit fd1a0c5

**Defects addressed:**
1. Retention pipeline `awk | while` masked awk failure
2. Test duplicated production logic instead of rendering actual template

**Changes:**
- Template: added `temporary_deletion_manifest` variable
- Template: modified cleanup() to remove both manifests
- Template: replaced pipeline with sequential stages: find > manifest; sort -o manifest; awk > deletion_manifest; while...; done < deletion_manifest
- Test: rewrote to render actual Jinja template by substituting 3 variables (DATA_DIR, BACKUP_DIR, RETENTION_COUNT)
- Test: added sqlite3 shim (copies DB for .backup, returns 'ok' for quick_check)
- Test: added gzip shim (pass-through)
- Test: covers missing DB exits 0, paths with spaces, newest-N retention, temp cleanup
- Test: added sort failure injection shim to prove nonzero propagation and cleanup

**Render test:**
```
$ cd ansible && ansible-playbook roles/uptime_kuma/tests/render-config.yml
PLAY [Validate Uptime Kuma role contract] *************************************
...
TASK [Assert backup script properties] ****************************************
ok: [localhost] => {
    "changed": false,
    "msg": "All assertions passed"
}
...
TASK [Validate backup script shell syntax] ************************************
ok: [localhost]
...
**Behavior test:**
```
$ ansible/roles/uptime_kuma/tests/test-backup-behavior.sh
Test 1: Missing database exits 0... PASS
Test 2: Full backup and newest-N retention... PASS
Test 3: Sort failure propagates and cleanup runs... PASS
...
**Shell syntax:**
```
$ sh -n ansible/roles/uptime_kuma/templates/uptime-kuma-backup.sh.j2
(no output = PASS)
```

**Kuma playbook syntax:**
```
$ cd ansible && ansible-playbook kuma.yaml --syntax-check
playbook: kuma.yaml
```

**Commit:**
```
fd1a0c5 fix(kuma): eliminate retention pipeline and test with rendered template
```

**Concerns:** None. All defects addressed, all tests pass.
